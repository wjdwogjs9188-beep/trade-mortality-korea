...
#
# Author: (공동저자 mode, 2026-05-07) PROJ <- "C:/Users/82103/Downloads/trade_mortality_korea"
DATA_PKG <- file.path(PROJ, "8_submission/paper_v01_submission")
HIRA_CW <- file.path(PROJ, "1_codebooks/hira_sgguCd_to_hcode_crosswalk.csv") cat("[Stage 0] Environment\n")
suppressPackageStartupMessages({ library(arrow); library(dplyr); library(tidyr); library(sandwich)
}) # ============================================================
...
iv_main <- arrow::read_parquet(file.path(DATA_PKG, "02_bartik_iv/iv_z_x_bilateral.parquet")) mortality$mort_rate <- mortality$deaths / pmax(mortality$pop_wa, 1)
mortality$log_mort <- log(mortality$mort_rate + 1e-6) build_panel_ld <- function(mort, base_yrs, end_yrs, iv_z) { mb <- mort %>% filter(year %in% base_yrs) %>% group_by(h_code, outcome_group) %>% summarise(b = mean(log_mort, na.rm = TRUE), .groups = "drop") me <- mort %>% filter(year %in% end_yrs) %>% group_by(h_code, outcome_group) %>% summarise(e = mean(log_mort, na.rm = TRUE), .groups = "drop") p <- mb %>% inner_join(me, by = c("h_code", "outcome_group")) %>% mutate(d_log_asr = e - b) %>% inner_join(iv_z, by = "h_code") p$sido_code <- substr(as.character(p$h_code), 1, 2) finite_z <- is.finite(p$z_x) p <- p[finite_z, ] p
} panel <- build_panel_ld(mortality, 1997:1999, 2018:2022, iv_main)
panel_despair <- panel[panel$outcome_group == "despair_total", ]
panel_despair$z_x_std <- as.numeric(scale(panel_despair$z_x_per_worker)) main_h_codes <- unique(panel_despair$h_code)
...
cat(sprintf(" HIRA crosswalk columns: %s\n", paste(colnames(hira_cw), collapse = ", "))) # h_code column 식별 (가능 한 column name: h_code, hcode, mapped_h_code 등)
hcode_col <- intersect(colnames(hira_cw), c("h_code", "hcode", "mapped_h_code", "matched_h_code"))[1]
if (is.na(hcode_col) || length(hcode_col) == 0) { cat(" ⚠️ h_code column not found by standard names. Inspecting first row:\n") cat(sprintf(" %s\n", paste(head(hira_cw, 1), collapse = " | "))) stop("Manual column identification needed — adjust hcode_col.")
}
cat(sprintf(" Using h_code column: '%s'\n", hcode_col)) hira_h_codes <- as.character(unique(hira_cw[[hcode_col]]))
...
n_only_hira <- length(setdiff(hira_h_codes, main_h_codes_str)) cat(sprintf(" Main only (in 221, NOT in HIRA): n = %d sigungu\n", n_only_main))
...
cat(sprintf(" panel_intersection rows: %d\n", nrow(panel_intersection))) fit_int <- lm(d_log_asr ~ z_x_std, data = panel_intersection)
beta_int <- coef(fit_int)[2] # HC1 SE
vcov_hc1 <- sandwich::vcovHC(fit_int, type = "HC1")
se_hc1 <- sqrt(diag(vcov_hc1))[2]
t_hc1 <- beta_int / se_hc1 # Cluster-province SE (G = number of unique sido)
...
for (c in unique_clusters) { idx <- which(cluster_id == c) Xc <- X[idx, , drop = FALSE]; rc <- resid_int[idx] s_c <- crossprod(Xc, rc); meat <- meat + tcrossprod(s_c)
}
G_factor <- (G / (G - 1)) * ((N - 1) / (N - K))
vcov_cluster <- bread %*% meat %*% bread * G_factor
se_cluster <- sqrt(diag(vcov_cluster))[2]
t_cluster <- beta_int / se_cluster p_hc1 <- 2 * pt(-abs(t_hc1), df = N - K)
p_cluster <- 2 * pt(-abs(t_cluster), df = G - 1) cat(sprintf(" β_intersection = %.6f\n", beta_int))
...
se_hc1_full <- sqrt(diag(vcov_hc1_full))[2] cat(sprintf(" Full sample (n=%d): β = %.6f, HC1 SE = %.6f\n", nrow(panel_despair), beta_full, se_hc1_full))
