...
## § 5. Sample, panel, IV (Pre-draft 4 정정 — 추가 검증 후 정정 회복)

### 5.4 ADH-8 robustness IV (Switzerland 포함 정상)

> **v4.4 patch (R-A 의 lapse 인정)**: 직전 v4.4 commit 의 "ADH-7 (Switzerland 누락)" 표기 → **"ADH-8 (8개국 모두 포함)"** 회복. trade_mortality_raw 의 통합 csv (Switzerland 부족) 만 본 R-A 의 lapse — 본 paper build 코드 (`2_scripts/bartik/04_bartik_iv_build.py`) 의 ADH_COUNTRIES dict 검증 결과 8개국 (Switzerland 포함) 모두 사용.

**검증 결과**:
- 본 paper build source = `trade_mortality_korea/0_raw/comtrade_adh_china/` × 8국 25 시점
- ADH 8 country ISO codes (build 코드 명시): AU + **CH** (Switzerland) + DE + DK + ES + FI + JP + NZ
- Switzerland CH_2000.csv ~ CH_2024.csv 25 시점 모두 보유 + build 사용

**ADH 2013 의 8 OECD**: AU, **CH** (Switzerland), DE, DK, ES, FI, JP, NZ — **본 paper 모두 포함** ✅
...
| ADH-8 published precedent (Lang 2018) | 8 (Switzerland 포함) | 18.77 | ≈3.32 | 2.06 | **본 paper 와 동일 8 OECD list, 동일 protocol** |

**Lang 2018 비교 framing 정상 회복**:
- Lang 2018 = 8 OECD (Switzerland 포함)
- 본 paper = 8 OECD (Switzerland 포함, 동일 list)
- 두 paper 의 IV 정의 정확히 일치 (8 OECD 동일) → first-stage F 직접 비교 가능 (Lang 18.77 vs 본 paper 19.65, 거의 동일 strength)

→ § 8.14 limitation 제거 (Switzerland 누락 한계 해당 안 됨)
→ § 12.5 P10 closed (사용자 측 Comtrade Switzerland fetch 불필요)
...
### 8.14 (REMOVED v4.4 patch) — ADH-7 (Switzerland 누락) 한계

> **v4.4 patch (R-A 의 lapse 인정)**: 본 § 8.14 limitation 항목 제거. trade_mortality_raw 의 통합 csv 만 본 R-A 의 lapse — 본 paper build 코드 (`04_bartik_iv_build.py`) 의 ADH_COUNTRIES dict 검증 결과 Switzerland 정상 포함. ADH-8 IV 정상.
...
| ~~**P10**~~ (CLOSED v4.4 patch) | ~~Comtrade Switzerland fetch~~ — **build 코드 검증 결과 이미 포함, 작업 불필요** | — | 0 | **closed** |
