...
CHAIN = RAW_DIR / "kosis_business_survey/ksic_version_concordance"
...
def main():
    ...
    # 8↔9 mapping (xls)
    wb = xlrd.open_workbook(str(CHAIN / "8차_9차개정 연계표.xls"))
    ...
    # 9↔10 mapping (xlsx)
    wb = openpyxl.load_workbook(str(CHAIN / "KSIC연계표(9차_10차).xlsx"), read_only=True, data_only=True)
    ...
    wb = openpyxl.load_workbook(str(CHAIN / "한국표준산업분류 제11차-제10차 연계표.xlsx"), read_only=True, data_only=True)
