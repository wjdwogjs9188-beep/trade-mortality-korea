...
#'
#' ```{r, comment = "#>", collapse = TRUE}
#' fixest::feols(y ~ i(year) | 0 | x ~ g, data = industry_df,
#' weights = ~ s_n, vcov = "hc1")
#' ```
#' Conditional shift-share IV with clustered standard errors
#'
#' ```{r, comment = "#>", collapse = TRUE}
#' fixest::feols(y ~ i(year) | 0 | x ~ g, data = industry_df[g < 45, ],
...
#'
#' ```{r, comment = "#>", collapse = TRUE}
#' fixest::feols(y ~ i(year) | 0 | z ~ g, data = industry_df,
#' weights = ~ s_n, vcov = "hc1")
#' ```
#'
#' Shock-level balance check
#' ```{r, comment = "#>", collapse = TRUE}
#' fixest::feols(l_sh_routine33 ~ g + i(year), data = industry_df,
#' weights = ~ s_n, vcov = "hc1")
#' ```
#'
...
ssaggregate = function(
 ...
 keep_vars = keep_vars[
 keep_vars %in% c("location_ids", t) | grepl(s, keep_vars)
 ]

 shares = data[,.SD,.SDcols = keep_vars]

 shares = data.table::melt(
 shares,
 ...
 rsq = fixest::feols(fixest::xpd(lhs = s, rhs = controls), data = temp) |>
 fixest::r2(type = "r2")
 if (is.na(rsq)) {
 rsq = 1
 }
 ...
 if (!addmissing) {
 warning(
 "You are in the incomplete share case (the sum of exposure shares varies) and you have not controlled for the sum of shares. You should either include the missing industry or (better in most cases) add the sum-of-share control. Otherwise the shock-level IV coefficient does not equal the shift-share IV."
)
 }
 }
 }
 ...
 # Merge in shares
 if (wideformat) {
 data[,.SD,.SDcols =!grepl(s, colnames(data))]
 }
 if (is.null(t)) {
