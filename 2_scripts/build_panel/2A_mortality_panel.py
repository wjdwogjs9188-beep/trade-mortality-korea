...
def main() -> None:
    ...
    if not pd.api.types.is_string_dtype(sample["사망원인_104항목분류코드"]):
        sys.exit(f"FAIL V1 dtype: cause_104 not string (got {sample['사망원인_104항목분류코드'].dtype})")
    ...
    # === V2 mutual exclusivity check (FAIL-fast) ===
    ...
    if len(long_all) != sum_check:
        sys.exit(f"FAIL V2 mutual exclusivity: long={len(long_all):,} != valid_sum={sum_check:,}")
    ...
    if len(bad) > 0:
        sys.exit(f"FAIL V2 mutual exclusivity: causes mapping to multiple groups: {bad.to_dict()}")
