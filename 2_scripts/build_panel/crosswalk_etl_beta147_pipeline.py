...
직전 R-A 의 cumulative crosswalk 정정 + ETL 재실행 + β_147 재추정의
cumulative result 를 사용자 측에서 verify + 재현 가능.

실행:
    Spyder: File → Open → 본 script → F5
    또는: %runfile C:/Users/82103/Downloads/trade_mortality_korea/2_scripts/build_panel/crosswalk_etl_beta147_pipeline.py --wdir
    또는 cmd: cd C:\\Users\\82103\\Downloads\\trade_mortality_korea && python 2_scripts\\build_panel\\crosswalk_etl_beta147_pipeline.py

출력:
    - console: 6-step cumulative pipeline output (~150 lines)
    - 3_derived/hira_atc4_panel.parquet (long, 1,640 rows)
    - 3_derived/hira_atc4_panel_wide.parquet (wide, 325 rows)
    - 1_codebooks/intersection_main_hira_h_codes.csv (147 sigungu)

주의:
    - HIRA crosswalk 의 220001 → 23090 정정은 직전 turn R-A 가 commit 완료.
      본 script 는 정정 verify 만 진행 (재정정 X).
    - statsmodels 부재 시 numpy/scipy 수동 fallback 자동 적용.
...
WA_CODES = [130, 150, 160, 180, 190, 210, 230, 260]  # KOSIS C3 working-age 25-64
...
try:
    ...
    HAS_SM = False
    print("[INFO] statsmodels 부재 — numpy/scipy 수동 fallback 사용\n")


def fit_ols_hc1(X, y):
    """OLS + HC1 SE 의 numpy/scipy 수동 form (statsmodels 부재 시 fallback)

    statsmodels version 위 fit.resid / X_const 의 ndarray vs Series ambiguity 우회 위해
    np.asarray 로 numpy 변환 한 번에 적용.
    """
    if HAS_SM:
        X_const = sm.add_constant(X)
        fit = sm.OLS(y, X_const).fit(cov_type='HC1')
        # numpy 변환으로 statsmodels version 영역 ambiguity 우회
        params = np.asarray(fit.params)
        bse = np.asarray(fit.bse)
        tvalues = np.asarray(fit.tvalues)
        resid = np.asarray(fit.resid)
        X_arr = np.asarray(X_const)
        return {
            'beta': float(params[1]),
            'se_hc1': float(bse[1]),
        ...
        return {
            ...
            'se_hc1': float(se_hc1),
            't_hc1': float(beta_hat[1] / se_hc1),
            'resid': resid,
            'X': X_const
        }


def compute_cluster_se(X, resid, cluster_id, K=2):
    """Cluster-province SE 의 수동 form"""
    G = cluster_id.nunique()
    N = len(resid)
    bread = np.linalg.inv(X.T @ X)
    meat = np.zeros((K, K))
    for c in cluster_id.unique():
        idx = (cluster_id == c).values if hasattr(cluster_id, 'values') else (cluster_id == c)
        Xc = X[idx, :]
        rc = resid[idx]
        s_c = Xc.T @ rc
    ...
    return np.sqrt(np.diag(vcov_cluster))[1], G
...
if len(target) == 0:
    print('\n  ⚠️  220001 row 부재 — crosswalk 영역 verify 필요')
    sys.exit(1)

t = target.iloc[0]
if pd.isna(t['h_code']) or t['status'] == 'UNMATCHED':
    print('\n  ⚠️  220001 정정 미반영 — h_code NaN 또는 status=UNMATCHED')
    print('  HIRA crosswalk 의 220001 row 를 23090.0 / MATCH 으로 정정 후 재실행 필요')
    sys.exit(1)
elif int(float(t['h_code'])) == 23090 and t['status'] == 'MATCH':
    print('\n  ✅ 220001 → 23090 MATCH 정상 commit')
else:
    print(f'\n  ⚠️  예상 외 h_code 영역: {t["h_code"]} / {t["status"]}')

match_n = (cw['status'] == 'MATCH').sum()
...
print(f'  year distribution: {raw["year"].value_counts().sort_index().to_dict()}')
...
print(f'  crosswalk dict size: {len(cw_dict)} (= 167 + 1 미추홀구)')
...
# Aggregation: sgguCd × year × atc4 sum (insupTp + recuCl + 12 month all)
agg = mapped.groupby(['h_code', 'year', 'atcStep4Cd'])['totUseQty'].sum().reset_index()
agg.columns = ['h_code', 'year', 'atc4', 'prescription_count']
print(f'  agg rows: {len(agg)} (expected 5 × 168 × 2 = 1,680, sparse cells reduce)')

# KOSIS pop working-age 25-64
...
print(f'  KOSIS pop agg: {len(agg_pop):,} rows')

# Join
agg['h_code_str'] = agg['h_code'].astype(str).str.zfill(5)
panel_long = agg.merge(agg_pop, on=['h_code_str', 'year'], how='left')
panel_long['prescription_rate_per_100k'] = (
    panel_long['prescription_count'] / panel_long['working_age_pop_25_64'].replace(0, np.nan)
) * 1e5

# main 221 sigungu identification (mortality + IV long-difference build)
...
main_221 = set(panel_despair['h_code'].astype(str).str.zfill(5).unique())
...
print(f'  23090 (미추홀구) inclusion: {"23090" in new_intersect} ✅')

# intersection csv 갱신
intersection_df = pd.DataFrame({
    'h_code': [int(h) for h in new_intersect],
    'in_main_221': True,
    'in_hira_168': True
})
intersection_df.to_csv(INTERSECT, index=False, encoding='utf-8')
print(f'  intersection_main_hira_h_codes.csv 갱신: n={len(intersection_df)}')

# Panel long flag
intersect_set = set(new_intersect)
panel_long['in_intersection_147'] = panel_long['h_code_str'].isin(intersect_set)

# Write parquet
panel_long_out = panel_long[[
    'h_code', 'year', 'atc4', 'prescription_count',
    'working_age_pop_25_64', 'prescription_rate_per_100k', 'in_intersection_147'
]].copy()
...
print(f'  long parquet commit: n={len(panel_long_out):,} rows')
...
print(f'    HC1 SE = {full_fit["se_hc1"]:.6f}, t = {full_fit["t_hc1"]:.4f}')

# Intersection 147
panel_int = panel_despair[panel_despair['h_code'].astype(int).isin(set(intersection_df['h_code']))].copy()
print(f'\n  Intersection 147 sample (n={len(panel_int)}):')

int_fit = fit_ols_hc1(panel_int['z_x_std'].values, panel_int['d_log_asr'].values)
print(f'    β_147 = {int_fit["beta"]:.6f}')
print(f'    HC1 SE = {int_fit["se_hc1"]:.6f}, t = {int_fit["t_hc1"]:.4f}')

# Cluster-province SE (G=13 sido)
...
print(f'    Cluster-province (G={G}) SE = {se_cluster:.6f}, t = {t_cluster:.4f}, p = {p_cluster:.4f}')
...
# ============================================================
# Step 5 — 직전 R-A sandbox 결과와 cumulative 정합 확인
# ============================================================
print('\n' + '=' * 70)
print('Step 5 — 직전 R-A sandbox 결과 cumulative 정합 verify')
print('=' * 70)
expected = {
    'β_221': -0.127947,
    'β_147': -0.154584,
...
for k in expected:
    e = expected[k]
    a = actual[k]
    match = abs(e - a) < 0.01 * abs(e) if abs(e) > 0.01 else abs(e - a) < 0.001
    flag = '✅' if match else '❌'
    if not match:
        all_match = False
    print(f'  {k:<20} {e:>14.4f} {a:>14.4f} {flag:>8}')

if all_match:
    print(f'\n  ✅ 모든 metric R-A sandbox 결과와 cumulative 정합 confirmed')
else:
...
# 2. Schema
expected_long_cols = ['h_code', 'year', 'atc4', 'prescription_count',
                     'working_age_pop_25_64', 'prescription_rate_per_100k', 'in_intersection_147']
chk_long = pd.read_parquet(OUT_DIR / "hira_atc4_panel.parquet")
schema_ok = set(expected_long_cols).issubset(set(chk_long.columns))
print(f'  [2] long schema OK: {schema_ok}')

# 3. distinct h_code
print(f'  [3] distinct h_code: {chk_long["h_code"].nunique()} (expected 168)')

# 4. intersection True count
print(f'  [4] in_intersection_147 True rows: {chk_long["in_intersection_147"].sum()} (expected ≈ 1,440)')

# 5. NaN ratio
nan_rate = chk_long['prescription_rate_per_100k'].isna().mean()
print(f'  [5] prescription_rate_per_100k NaN ratio: {nan_rate*100:.2f}% (expected < 5%)')

# 6. Year coverage
yc = chk_long.groupby('h_code')['year'].nunique().describe()
print(f'  [6] year coverage min/median/max: {int(yc["min"])}/{int(yc["50%"])}/{int(yc["max"])}')
