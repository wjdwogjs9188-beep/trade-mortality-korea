"""Phase 2 sub-task 2.2 audit script — explore-data + validate-data 합본 (Windows path) 본 script 는 사용자 Spyder 환경 에서 직접 실행 가능.
실행: F5 또는 python audit_2_2_hira.py
출력: console (audit cycle 결과 와 동일)
"""
import pandas as pd
import numpy as np
...
SIGUNGU_CW = PROJ / "1_codebooks" / "sigungu_crosswalk.csv" # Load
...
for atc in sorted(pl['atc4'].unique): s10 = pl[(pl['atc4']==atc) & (pl['year']==2010)]['prescription_rate_per_100k'].median s19 = pl[(pl['atc4']==atc) & (pl['year']==2019)]['prescription_rate_per_100k'].median if pd.notna(s10) and pd.notna(s19) and s10 > 0: dlog = np.log(s19) - np.log(s10) growth = (s19/s10 - 1) * 100 print(f' {atc:<8} {s10:>14,.0f} {s19:>14,.0f} {dlog:>10.4f} {growth:>11.1f}%') # Cross-ATC4 correlation
...
print('=' * 70) expected_intersect = pd.MultiIndex.from_product([ sorted(intersect['h_code'].astype(int).unique), [2010, 2019], sorted(pl['atc4'].unique)
...
print(f' expected: {len(expected_intersect):,}, actual: {len(actual_intersect):,}, sparse: {len(sparse):,}') # Sparse 6 sigungu identification
sparse_h = sorted(sparse['h_code'].unique)
print(f'\n Sparse 6 sigungu 의 substantive 식별:')
for h in sparse_h: sub = sigungu_cw[sigungu_cw['h_code']==h].drop_duplicates(['h_code']) if len(sub) > 0: r = sub.iloc[0] sparse_year = sparse[sparse['h_code']==h]['year'].unique event_note = r['event_note'] if pd.notna(r['event_note']) else '(no event_note)' print(f' h_code={h} year={sparse_year[0]}: {r["sido_name"]} {r["h_name"]}') print(f' event_note: {event_note}') # Aggregation cross-check (5 random cells)
...
raw_agg['h_code'] = raw_agg['h_code'].astype(int) np.random.seed(42)
...
for _, r in intersect_rows.iloc[sample_idx].iterrows: h, y, a = int(r['h_code']), int(r['year']), r['atc4'] panel_count = r['prescription_count'] raw_sub = raw_agg[(raw_agg['h_code']==h) & (raw_agg['year']==y) & (raw_agg['atcStep4Cd']==a)] raw_sum = raw_sub['totUseQty'].sum flag = 'OK' if abs(panel_count - raw_sum) < 1 else 'MISMATCH' print(f' h={h} y={y} a={a}: panel={panel_count:,.0f} vs raw_sum={raw_sum:,.0f} [{flag}]') # Selection bias intersection vs non-intersection
print(f'\n=== 6. Selection bias: intersection 146 vs non-intersection 21 ===')
print(f' {"ATC4":<8} {"int median":>14} {"non-int median":>16} {"ratio":>10}')
for atc in sorted(pl['atc4'].unique): s_int = pl[(pl['atc4']==atc) & (pl['in_intersection_146'])]['prescription_rate_per_100k'].dropna s_non = pl[(pl['atc4']==atc) & (~pl['in_intersection_146'])]['prescription_rate_per_100k'].dropna if len(s_int) > 0 and len(s_non) > 0: ratio = s_int.median / s_non.median print(f' {atc:<8} {s_int.median:>14,.0f} {s_non.median:>16,.0f} {ratio:>10.3f}') # UNMATCHED sgguCd
...
for atc in sorted(raw['atcStep4Cd'].unique): sub = raw[raw['atcStep4Cd']==atc] ratio = sub['msupUseAmt'].median / max(sub['totUseQty'].median, 1) print(f' {atc}: ratio = {ratio:.2f}') print('\n=== Audit complete ===')
